...
const MAIN := preload("res://scripts/main_v077.gd")
...
func _fail(message: String) -> void:
	failed = true
	push_error("CAMERA_TRANSFORM_077_SMOKE_FAIL: %s" % message)
...
func _run() -> void:
	...
	if str(app.get("VERSION_077")) != "0.5.21":
		_fail("runtime version is not 0.5.21")
	...
	if app.transform_panel_v077 == null or app.transform_toggle_v077 == null:
		_fail("compact transform panel was not created")
	...
	if failed:
		quit(1)
		return
	print("CAMERA_TRANSFORM_077_SMOKE_OK: unified transform, analog camera translation/elevation, Center focus and scroll-safe menu controls verified")
	quit(0)
